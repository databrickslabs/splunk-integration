...
# MAGIC <br>This notebook is used where the syslogs format is according to RFC-3164
# MAGIC <br>The Syslog Logs Path and Region Name can be obtained from AWS account admin.
# MAGIC 1. **Syslog Logs Path** : The folder in the S3 bucket from which to collect data. It should be of the form `s3://<bucket-name>/<path_to_syslog_folder>/*`.
# MAGIC <br>**Example:** Syslogs Path : `s3://mybucket/my_syslog_folder/*`
# MAGIC 2. **Delta Output Path** : The DBFS or S3 path where the parsed data files should be stored. Ensure that this path is either empty(contains no data files) or is not a pre-existing path or does not contain any data that does not follow Syslogs schema (schema as specified in cmd 5).
...
# MAGIC %md
# MAGIC ## Defining schema for Syslogs (RFC-3164)
# MAGIC Reference : [Syslog Format as logged by Fluentd](https://docs.fluentd.org/parser/syslog#rfc-3164-pattern) and [Example](https://docs.fluentd.org/parser/syslog#rfc-3164-log)
