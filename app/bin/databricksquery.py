...
class DatabricksQueryCommand(GeneratingCommand):
    ...
    def generate(self):
        ...
        try:
            ...
            def handle_dbsql_method(row_limit):
                ...
                for res in response:
                    if res.get("id") == self.warehouse_id:
                        ...
                        if res.get("state").lower() != "running":
                            raise Exception(
                                "Ensure that the SQL warehouse is in running state. "
                                "Current SQL warehouse state is {}.".format(res.get("state"))
                            )
                        break
            ...
            if not self.cluster and not self.warehouse_id:
                ...
                if dbquery_type == "dbsql":
                    ...
                    for event in handle_dbsql_method(row_limit):
                        yield event
                elif (
