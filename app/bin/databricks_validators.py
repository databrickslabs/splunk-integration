...
class ValidateDatabricksInstance(Validator):
    ...
    def validate_pat(self, data):
        ...
        databricks_personal_access_token = data.get("databricks_personal_access_token")
        return self.validate_db_instance(databricks_instance, databricks_personal_access_token)
    ...
    def validate_aad(self, data):
        ...
        access_token = utils.get_aad_access_token(
            self._splunk_session_key, const.USER_AGENT_CONST.format(self._splunk_version),
        ...
        if valid_instance:
            ...
            data["databricks_personal_access_token"] = ""
            return True
        else:
            return False
    ...
    def validate_db_instance(self, instance_url, access_token):
        ...
        headers = {
            ...
            "User-Agent": const.USER_AGENT_CONST.format(self._splunk_version),
        }
        try:
    ...
    def validate(self, value, data):
        ...
        if auth_type == "PAT":
            if (not (data.get("databricks_personal_access_token", None)
                     and data.get("databricks_personal_access_token").strip())
                    ):
        ...
        self._splunk_version = ServerInfo(self._splunk_session_key).version
        if auth_type == "PAT":
